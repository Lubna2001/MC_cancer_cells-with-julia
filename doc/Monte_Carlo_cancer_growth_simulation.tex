...
important role in shaping growth dynamics \cite{Chkhaidze2019}.
...
are well suited to capture spatial heterogeneity and emergent behavior arising from cell–cell interactions and environmental constraints \cite{Wang2014} \cite{Drasdo2005} \cite{Metzcar2019} .

In this project, tumor growth is modeled using an agent-based Monte Carlo approach, an approach that has been used in previous studies
...
increases toward the lattice boundary, mimicking diffusion-limited oxygen supply as defined in Equation~\eqref{eq:oxygen}.
...
Cell division is attempted by placing a daughter cell in a neighboring empty site. If no empty neighbor is available,
a pushing attempt is made with probability $p_{\text{push}}$. A lattice direction is chosen at random, and if an empty site 
is found along that direction, the cells along that direction are shifted to create space for the daughter cell. Cancer cells are assigned a higher probability to push neighboring
...
A founder cancer cell occurs by introducing a mutation event in a normal cell that has at least one neighboring empty lattice site. 
...
to maintain a finite probability of division even in hypoxic environments. For the result of our simulations, a minimum oxygen level 
...
Here, $O \in [O_{\min},1]$ denotes the local oxygen concentration, with $O_{\min}=0.35$ in our current simulations.
...
$O_{2}^{\mathrm{mut}}$is the local oxygen concentration at the mutation site. The founder cancer cell is marked by a yellow circle.
...
on growth happened through the pushing mechanisim. Cancer growth in this case remains significantly reduced compared to earlier mutation times, highlighting 
...
Instead, the key difference is the local density of normal cells at $t_{\mathrm{mut}}$.
This is quantified by the function $N_n(t,\mathrm{seed})$, representing the number of normal cells at time $t$ for a given random seed.
For $t_{\mathrm{mut}} = 120$, we find
\[
N_n(120,6) = 1225,
\qquad
N_n(120,10) = 288.
...
The much lower crowding at  $t_{\mathrm{mut}}$ in seed 10 allows the cancer population to expand more easily, especially given its higher
division and pushing probabilities and lower death probability compared to normal cells \ref{table:parameters}.
